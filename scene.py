...
class Scene:
    ...
    @staticmethod
    def generate_chunk(offset):
        blocks = []
        for x in range(CHUNK_WIDTH + 1):
            for y in range(CHUNK_HEIGHT + 1):
                blocks.append(Block(int(offset[0]) + x, int(offset[1] + y)))
        return blocks
    ...
    def render(self):
        ...
        for chunk in self.find_player_chunk():
            self.game.screen.fblits(
                [(block.image, (int(block.position[0]) - self.camera_scroll[0], int(block.position[1]) - self.camera_scroll[1]))
                 for block in chunk])
