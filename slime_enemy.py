...
class SlimeEnemy(Entity):
    def __init__(self):
        idle_image = pygame.transform.scale(
            surface=pygame.image.load(file="assets/textures/slime.png").convert_alpha(),
            size=(BLOCK_SIZE, BLOCK_SIZE),
        )
        right_image = pygame.transform.scale(
            surface=pygame.image.load(
                file="assets/textures/right_slime.png"
            ).convert_alpha(),
            size=(BLOCK_SIZE, BLOCK_SIZE),
        )
        left_image = pygame.transform.scale(
            surface=pygame.image.load(
                file="assets/textures/left_slime.png"
            ).convert_alpha(),
            size=(BLOCK_SIZE, BLOCK_SIZE),
        )
        super().__init__(
        ...
        self.__attack_distance = (
            10 * BLOCK_SIZE
        )  # Define the distance at which the enemy starts attacking

        self.__direction_timer = 0
        self.__random_direction = None

        self._directions = {"left": False, "right": False, "idle": False}

        self.__attack_cooldown = 0

    def __random_movement(self):
        if self.__direction_timer <= 0:
            self.__direction_timer = random.randint(
                100, 150
            )  # Number of frames before the enemy changes direction
    ...
    def attack_update(self, player):
        ...
        if (
            distance > self.__attack_distance
        ):  # If player is out of attack range, perform random movement
            ...
            if self._rect.colliderect(player.get_rect()):
                if self.__attack_cooldown <= 0:
                    ...
                    player_damage = pygame.mixer.Sound(
                        file="assets/sound/player_damage.mp3"
                    )
