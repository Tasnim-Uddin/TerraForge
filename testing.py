...
class TestGame(unittest.TestCase):
    ...
    def test_validate_password_text_input(self):
        ...
        self.assertFalse(
            self.game.validate_password_text_input(
...
class TestScene(unittest.TestCase):
    ...
    def test_generate_chunk(self):
        ...
        # Ensure that the chunk contains some blocks
        self.assertTrue(len(self.scene._Scene__chunks[key]) > 0)

        # Ensure that the chunk contains expected block types based on conditions
        for block_position, block_type in self.scene._Scene__chunks[key].items():
            ...
            height_noise = int(
                (self.scene.world_seed.noise2(x=real_x * 0.003, y=0) * 200)
                // BLOCK_SIZE
                * BLOCK_SIZE
            )
            cave_value = 0.05
            ...
            if real_y + height_noise == 0:
                ...
                if (
                    self.scene.lower_cave_threshold
                    < cave_value
                    < self.scene.upper_cave_threshold
                ):
                    self.assertIn(
                        block_type,
                        [
    ...
    def test_check_door(self):
        ...
        self.assertEqual(
            self.scene._Scene__chunks[(0, 0)][(0, 0)], "top_oak_door_closed"
        )
        self.assertEqual(
            self.scene._Scene__chunks[(0, 0)][(0, 1)], "bottom_oak_door_closed"
        )
        ...
        self.assertEqual(
            self.scene._Scene__chunks[(0, 0)][(0, 1)], "bottom_oak_door_open"
        )
        ...
        # Assert expected changes
        self.assertNotEqual(
            self.scene._Scene__chunks[(0, 0)][(0, 0)], "top_oak_door_open"
        )
        self.assertNotEqual(
            self.scene._Scene__chunks[(0, 0)][(0, 1)], "bottom_oak_door_open"
        )
    ...
    def test___break_block(self):
        ...
        self.scene._Scene__chunks = {(0, 0): {(10, 10): "stone"}}
        ...
        self.scene._Scene__chunks = {
            (0, 0): {
                (10, 10): "top_oak_door_closed",
                (10, 11): "bottom_oak_door_closed",
            }
        }
        self.scene._Scene__break_block(held_item="copper_pickaxe")
        self.assertNotIn((10, 10), self.scene._Scene__chunks[(0, 0)])
        ...
        self.scene._Scene__inventory._Inventory__inventory_items = {
            (0, 0): {"item": "copper_pickaxe", "quantity": 1},
        ...
        with patch("random.random", return_value=0.03):
            self.assertTrue(
                "apple",
                self.scene._Scene__inventory._Inventory__inventory_items[(0, 1)][
                    "item"
                ],
            )
            self.assertTrue(
    ...
    def test_load_world_from_json(self):
        ...
        with patch("os.path.exists", return_value=True):
            with patch(
                "builtins.open",
                mock_open(read_data=json.dumps(mock_serialized_world_data)),
            ) as mock_file:
                # Call the method
                ...
                assert (
                    world_seed_value == expected_world_seed_value
                ), "Loaded world seed value does not match expected data"
                assert (
                    chunks == expected_chunks
                ), "Loaded chunks do not match expected data"
...
class TestInventory(unittest.TestCase):
    ...
    def test_add_item(self):
        ...
        self.assertEqual(
            self.inventory._Inventory__inventory_items[(0, 3)]["item"], "dirt"
        )
        self.assertEqual(
            self.inventory._Inventory__inventory_items[(0, 3)]["quantity"], 1
        )

        # Test adding quantity to existing item
        self.inventory.add_item(item="dirt", quantity=2)
        self.assertEqual(
            self.inventory._Inventory__inventory_items[(0, 3)]["quantity"], 3
        )

    def test_remove_item(self):
        # Test removing item from inventory
        self.inventory._Inventory__inventory_items[(0, 3)] = {
            "item": "dirt",
            "quantity": 1,
        }
        ...
        self.assertIsNone(
            self.inventory._Inventory__inventory_items[(0, 3)]["quantity"]
        )
    ...
    def test_determine_crafting_table(self):
        ...
        self.inventory.determine_crafting_table(
            player_position, neighbour_chunk_offsets, crafting_table_positions
        )
    ...
    def test_determine_furnace(self):
        ...
        self.inventory.determine_furnace(
            player_position, neighbour_chunk_offsets, furnace_positions
        )
    ...
    def test_craft_item(self):
        ...
        with patch("builtins.print") as mock_print:
            ...
            mock_print.assert_called_once_with(
                "You don't have enough stick to craft torch"
            )

            # Assert that the inventory state remains unchanged after failed crafting
            self.assertEqual(
                self.inventory.get_item_quantity("stick"), 0
            )  # Should remain 0
            self.assertEqual(
                self.inventory.get_item_quantity("coal"), 0
            )  # Should remain 0
            self.assertEqual(
                self.inventory.get_item_quantity("torch"), 0
            )  # Should remain 0
    ...
    def test_save_inventory_to_json(self):
        ...
        with patch("builtins.open", mock_open()) as mock_file:
            ...
            inventory_path = os.path.join(
                PLAYER_SAVE_FOLDER, f"{self.inventory._Inventory__inventory_name}.json"
            )
    ...
    def test_load_inventory_from_json(self):
        ...
        with patch("os.path.exists", return_value=True):
            with patch(
                "builtins.open",
                mock_open(read_data=json.dumps(mock_serialized_inventory)),
            ) as mock_file:
                # Call the method
                ...
                inventory_path = os.path.join(
                    PLAYER_SAVE_FOLDER,
                    f"{self.inventory._Inventory__inventory_name}.json",
                )
                ...
                assert (
                    inventory_items == expected_inventory_items
                ), "Loaded inventory items do not match expected data"
...
class TestEntity(unittest.TestCase):
    ...
    def test_initialization(self):
        ...
        self.assertEqual(
            self.entity._directions,
            {"left": False, "right": False, "up": False, "idle": False},
        )
...
class TestPlayer(unittest.TestCase):
    ...
    def test_initialization(self):
        # Check if the player is properly Initialised
        self.assertIsInstance(self.player, Player)

    def test_set_velocity(self):
        # Test setting velocity when moving right
        self.player._directions = {
            "right": True,
            "left": False,
            "up": False,
            "idle": False,
...
class TestSlimeEnemy(unittest.TestCase):
    ...
    def test_attack_update(self):
        ...
        self.assertLessEqual(
            self.slime_enemy._SlimeEnemy__attack_cooldown, initial_cooldown
        )
...
class TestClient(unittest.TestCase):
    ...
    def test_authenticate_recovery_code(self, mock_post):
        ...
        # Ensure requests.post was called with correct URL and data
        mock_post.assert_called_once_with(
            "http://localhost:5000/authenticate_recovery_code",
